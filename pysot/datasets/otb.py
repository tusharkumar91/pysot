...
class OTBVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names,
            gt_rect, attr, query, load_img=False):
        super(OTBVideo, self).__init__(name, root, video_dir,
                init_rect, img_names, gt_rect, attr, query, load_img)
...
class OTBDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        query_folder = os.path.join(dataset_root, 'OTB_query_all')
        excluded_videos = ['ClifBar']

        # COPY Jogging images into Jogging-1 and Jogging-2 to cater to two textual descriptions by the same name
        # Rename Human4 directory to Human4-2 and do the same for the nlp annotation file
        ...
        for video in pbar:
            ...
            ## load query
            videoname = meta_data[video]['img_names'][0].split('/')[0]
            if videoname in excluded_videos:
                continue
            query = list(open(os.path.join(query_folder, '%s.txt' % videoname), 'r'))[0]
            query = query.strip()
